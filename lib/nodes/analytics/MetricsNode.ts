...
import { DateRangeSchema, MetricTypeSchema, TimeSeriesDataSchema } from "./schemas";
...
const generateMockData = (dateRange: { start: Date; end: Date }): TimeSeriesData[] => {
...
export class MetricsNode {
	...
	outputs = {
		timeSeriesData: new Output({
			...
			observable: combineLatest([this.inputs.dateRange, this.inputs.metric]).pipe(map(([dateRange]) => generateMockData(dateRange))),
		}),

	...
	const simulateUpload = async (file: UploadingFile) => {
		...
		for (let i = 1; i <= steps; i++) {
			...
			setFiles((prev) =>
				prev.map((f) =>
					f.id === file.id
						? { ...f, progress: (i / steps) * 100, status: "uploading" }
						: f,
				),
			);
		}
			...
			<DialogContent className="max-h-[90vh] overflow-y-auto p-0">
				<div className="flex flex-col h-[80vh]">
					<div
						{...getRootProps()}
						className={cn(
							"flex-1 border-2 border-dashed rounded-lg m-4 flex flex-col items-center justify-center gap-4 transition-colors",
							isDragActive
					...
					{files.length > 0 && (
						<div className="border-t p-4 gap-4 flex flex-col">
